  ...
  const run = async () => {
    ...
    try {
      ...
      if (res.content && res.content.length > 0) {
        setResponseOutput(res.content[0].text)
      } else {
    ...
    <section className="flex min-h-0 flex-1 flex-col gap-5 p-5 md:p-8"><div className="flex items-end justify-between"><div><p className="text-[11px] font-medium text-primary">Playground</p><h2 className="mt-1 text-[24px] font-semibold tracking-[-0.035em]">Try an idea</h2><p className="mt-1 text-[13px] text-muted-foreground">Shape a prompt, add your inputs, and see what happens.</p></div><div className="hidden items-center gap-2 text-[11px] text-muted-foreground sm:flex"><Zap className="size-3.5 text-primary" /> Ready when you are</div></div>
      ...
      <div className="mt-4 flex min-h-0 flex-1 items-center justify-center rounded-lg border border-dashed border-border bg-background/70 p-6 text-center">
        {loading ? (
          ...
          <div className="w-full h-full overflow-auto text-left text-[13px] leading-6 whitespace-pre-wrap font-mono">{responseOutput}</div>
        ) : ran ? (
      ...
      </div><div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-border pt-4 text-[11px] text-muted-foreground"><span>Tokens used <strong className="ml-1 font-medium text-foreground">—</strong></span><span>Latency <strong className="ml-1 font-medium text-foreground">—</strong></span><span>Model <strong className="ml-1 font-medium text-foreground">{currentModelName}</strong></span></div></div></div><div className="flex justify-end"><button onClick={run} disabled={!ready || loading} className="flex items-center gap-2 rounded-lg bg-primary px-5 py-2.5 text-[12px] font-semibold text-primary-foreground glow-hover transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40"><Play className="size-3.5 fill-current" />Run</button></div></section></main>{settingsOpen && <ModelSettings providers={providers} providerId={providerId} modelId={modelId} apiKey={apiKey} setProviderId={setProviderId} setModelId={setModelId} setApiKey={setApiKey} onClose={() => setSettingsOpen(false)} />}</div>
}
